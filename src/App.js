  ...
  function clickDoor(doorIndex) {
    ...
    setState({
      ...
      doors: state.doors.map((door, i) =>
        i !== doorIndex ? door : { ...state.doors[doorIndex], open: true }
      ),
    });
  }
  ...
  return (
    <div id="app">
      ...
      <div id="instructions">
        ...
        <div>
          Doors with a <strong>?</strong> will have their point values revealed
          when clicked.
        </div>
        <hr style={{ width: "200px" }} />
        {state.turns > 0 && (
          <div>
            Clicks Remaining: <strong>{state.turns}</strong>
          </div>
        )}
        <div>
          Points: <strong>{state.score}</strong>
        </div>
        {state.turns === 0 && (
